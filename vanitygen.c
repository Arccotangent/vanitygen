...
#define BN_MASK2 (0xfffffffffffffffL)
#endif
...
usage(const char *name)
{
    fprintf(stderr,
	    ...
	    "-X <version>  Generate address with the given version (applies for script addresses by default, unless locked by -Z. Sets private key version to 128 + <version> by default, unless locked by -x)\n"
	    "-x <version>  For advanced users, private key version (only changes private key version, nothing else)\n"
	    "-Z <version>  For advanced users, script (P2SH) address version (only changes P2SH version, nothing else)\n"
	    "-F <format>   Generate address with the given format (pubkey or script)\n"
	    "-P <pubkey>   Specify base public key for piecewise key generation\n"
	    "-B <amount>   Bulk address generation, generate a large amount of addresses regardless of prefix\n"
...
main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "vqnrik1eE:P:NTx:X:ZLF:t:h?f:o:s:")) != -1) {
	switch (opt) {
	case 'v':
	    verbose = 2;
	    break;
	...
	case 'X':
	    ...
	    printf("Set address version to %i\n", addrtype);
	    if (pset == 0)
	    {
		privtype = 128 + addrtype;
		printf("Private key version is default (address version + 128): %i\n", privtype);
	    }
	    if (sset == 0)
	    {
		scriptaddrtype = addrtype;
		printf("Script (P2SH) version is default (address version): %i\n", scriptaddrtype);
	    }
	    break;
	case 'x':
	    privtype = atoi(optarg);
	    printf("Set private key version to %i\n", privtype);
	    pset = 1;
	    break;
	case 'Z':
	    scriptaddrtype = atoi(optarg);
	    printf("Set script (P2SH) version to %i\n", scriptaddrtype);
	    sset = 1;
	    break;
	case 'F':
	    if (!strcmp(optarg, "script"))
	    {
		printf("Generating script (P2SH) address.\n");
		format = VCF_SCRIPT;
	    }
	    else if (strcmp(optarg, "pubkey"))
	    {
		printf("Generating regular pubkey (P2PKH) address.\n");
		format = VCF_PUBKEY;
	    }
	    else
	    {
		printf("Invalid format '%s', defaulting to regular pubkey (P2PKH) address generator.\n", optarg);
		format = VCF_PUBKEY;
	    }
	    break;
	case 'P': {
