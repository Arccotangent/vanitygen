...
usage(const char *progname)
{
    fprintf(stderr,
	    ...
	    "-H <hex key>  Encode a hexadecimal key to regular WIF base58 key and decimal key\n"
	    "-D <dec key>  Encode a decimal key to regular WIF base58 key and hex key, basically convert a number to private key\n"
	    "-X <version>  Address version\n"
	    "-x <version>  Private key version\n"
	    "-v            Verbose output\n"
	    "\nIf no options are specified, convert private key to address\n", 
	    version, progname);
}
...
main(int argc, char **argv)
{
    ...
    const char *key_tc = NULL;
    EC_KEY *pkey;
    int parameter_group = -1;
    int privtype = 128, addrtype = 0;
    int pkcs8 = 0;
    int pass_prompt = 0;
    int verbose = 0;
    int generate = 0;
    ...
    while ((opt = getopt(argc, argv, "h8E:ec:H:D:vGX:x:")) != -1) {
	switch (opt) {
	case '8':
	    pkcs8 = 1;
	    break;
    ...
    if (convert == 1) {
	//BIGNUM *bntmp = BN_new(), *bntmp2 = BN_new();
	BIGNUM *pkeytmp = BN_new();
	EC_KEY *pkey2;
	char privkey_buf[256];

	BN_hex2bn(&pkeytmp, key_tc);
	pkey2 = EC_KEY_new_by_curve_name(NID_secp256k1);
	EC_KEY_set_private_key(pkey2, pkeytmp);
	vg_encode_privkey(pkey2, privtype, privkey_buf);
	char* bndec = BN_bn2dec(pkeytmp);
	printf("Privkey (dec): %s\n", bndec);
	BN_clear_free(pkeytmp);
	EC_KEY_free(pkey2);
	printf("Privkey: %s\n", privkey_buf);
	return 0;
    }
	...
	BN_dec2bn(&pkeytmp, key_tc);
	pkey2 = EC_KEY_new_by_curve_name(NID_secp256k1);
	EC_KEY_set_private_key(pkey2, pkeytmp);
	vg_encode_privkey(pkey2, privtype, privkey_buf);
	printf("Privkey (hex): ");
	dumpbn(EC_KEY_get0_private_key(pkey2));
	BN_clear_free(pkeytmp);
	EC_KEY_free(pkey2);
	printf("Privkey: %s\n", privkey_buf);
	return 0;
